...
public class MessageReciver {
    public static void startReciverMessage() {
        try {
            Channel chanel = MQFactory.getChanel(Constants.RabbitConstants.MessageService2Im);
            //使用 exchangeDeclare 声明一个名为 MessageService2Im 的交换机，类型为 direct，
            // 表示消息将被路由到绑定到该交换机的特定队列。true 参数表示交换机是持久化的。
            chanel.exchangeDeclare(Constants.RabbitConstants.MessageService2Im, "direct", true);
            //使用 queueDeclare 声明一个名为 MessageService2Im 的队列。
            // 此队列是持久化的（true），即使 RabbitMQ 服务器重启，队列也会保留。
            chanel.queueDeclare(Constants.RabbitConstants.MessageService2Im,
                    true, false,false ,null );
            //将队列 MessageService2Im 绑定到同名的 MessageService2Im 交换机。
            // 绑定的路由键为空字符串（""），表示所有发往该交换机的消息都会路由到该队列。
            chanel.queueBind(Constants.RabbitConstants.MessageService2Im,Constants.RabbitConstants.MessageService2Im,"");
            //开始消费来自队列的消息。第二个参数 false 表示手动消息确认（在消息处理完毕后，客户端需要手动发送 ack）。
            chanel.basicConsume(Constants.RabbitConstants.MessageService2Im, false,
